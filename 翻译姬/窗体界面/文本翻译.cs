...
namespace 翻译姬 {
    public partial class 文本翻译 : 自定义Page {
                    ...
                    try {
                        ...
                        foreach (var 文件 in 处理中文件结构) {
                            文件.文件完成机翻 = () => {
                                ...
                                文件.写出();
                                数据中转.文本显示AppendLine($"已完成：{new FileInfo(文件.路径).Name}");
                            };
                        }
                        调用管理.文本机翻(机翻, true, 处理中文件结构);
                        if (机翻 == 机翻方式.不机翻) {
                            foreach (var 文件 in 处理中文件结构) {
                                文件.机翻后数据处理(机翻);
                                文件.写出();
        ...
        //Json转化而来后文本组和有效文本保存的文本对象指向的就不是同一个地址了，单独反向覆盖保留机翻状态
        ...
        private void 预显示Btn_Click(object sender, EventArgs e) {
            try {
                ...
                sb.AppendLine("*****************************************************************");
                foreach (var 文件 in 处理中文件结构) {
                    调用管理.文本机翻(机翻方式.不机翻, true, 文件);
                    //模拟机翻
                    var 译文组 = new List<文本>();
                    for (int i = 0; i < 文件.文本组.Length; i++) {
                        var 文本组 = 文件.文本组[i];
                        if (机翻接口Box.Text != "GPT") {
                            foreach (var 文本 in 文本组.文本) {
                                string text = 文本.原文;
                ...
                sb.AppendLine("*****************************************************************");
                string 预计字符 = "预计使用字符(102%)：" + (int)(总字符 * 1.02);
                if (机翻接口Box.Text == "GPT") {
